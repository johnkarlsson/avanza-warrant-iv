...
import IVHistoryChart from "./iv-history-chart.jsx";
import {
  cdf, npdf, bsPut, bsCall, bsCall1Sigma, bsPut1Sigma, bsVega, solveIV,
  LONG_MONTHS, SHORT_MONTHS, thirdFriday, parseWarrantExpiry,
} from "./bs-math.js";
...
export default function WarrantCalculator() {
  ...
  useEffect(() => {
    const update = () => {
      ...
      setVisiblePanels(w >= 2700 ? 4 : w >= 2024 ? 3 : w >= 1700 ? 2 : 1);
    };
    update();
  ...
  const canGoRight = activePanel < 4 - visiblePanels;
    ...
    >
      <style>{`
        ...
        *::-webkit-scrollbar-thumb:hover { background: #2a3050; }
        ...
        @media (min-width: 2024px) { :root { --panel-count: 3; } }
        @media (min-width: 2700px) { :root { --panel-count: 4; --arrow-gutter: 0px; } }
        .panel-container {
          display: flex;
          overflow-x: auto;
        ...
        @media (min-width: 2700px) { .nav-arrow { display: none; } }
      `}</style>

      {visiblePanels < 4 && canGoLeft && (
        <button
                  ...
                  >
                    ...
                    <td
                      style={{
                        ...
                        textAlign: "right",
                        color:
                          r.change === 0 ? "#fff" : r.change < 0 ? "#e53935" : "#4caf50",
                      }}
                    >
                      {r.change > 0 ? "+" : ""}
                      {r.change}%
